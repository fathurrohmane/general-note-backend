package com.elkusnandi.common.util

import io.jsonwebtoken.*
import io.jsonwebtoken.security.Keys
import io.jsonwebtoken.security.SignatureException
import java.util.*

object JwtUtil {

    private const val EXPIRATION_TIME = 1000 * 60 * 60; // 1 hour
    private const val EXPIRATION_TIME_REFRESH_TOKEN = 1000 * 60 * 60 * 27 * 7; // 1 hour
    private const val SECRET_KEY =
        ""

    fun generateToken(userId: String, tokenId: String, isRefreshToken: Boolean = false): String =
        Jwts
            .builder()
            .setSubject(userId)
            .setIssuedAt(Date(System.currentTimeMillis()))
            .setId(tokenId)
            .setExpiration(Date(System.currentTimeMillis() + if (isRefreshToken) EXPIRATION_TIME_REFRESH_TOKEN else EXPIRATION_TIME))
            .signWith(Keys.hmacShaKeyFor(SECRET_KEY.toByteArray()), SignatureAlgorithm.HS256)
            .compact()

    fun getAllClaims(token: String): Claims =
        Jwts.parserBuilder()
            .setSigningKey(SECRET_KEY.toByteArray())
            .build()
            .parseClaimsJws(token).body

    fun checkAndGetClaims(token: String): Claims? {
        try {
            return getAllClaims(token)
        } catch (_: SignatureException) {

        } catch (_: MalformedJwtException) {

        } catch (_: ExpiredJwtException) {

        } catch (_: UnsupportedJwtException) {

        } catch (_: IllegalArgumentException) {

        }
        return null
    }

}